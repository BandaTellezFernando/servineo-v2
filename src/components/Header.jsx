...
import PerfilModal from "./PerfilModal"; // NUEVO: Importamos el Perfil

export default function Header() {
  const [isRegistroOpen, setIsRegistroOpen] = useState(false);
  const [isLoginOpen, setIsLoginOpen] = useState(false);
  const [isPerfilOpen, setIsPerfilOpen] = useState(false); // NUEVO: Estado del Perfil
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            {usuarioActual ? (
              ...
              // NUEVO: Agregamos el onClick para abrir el modal de perfil
              <button 
                onClick={() => setIsPerfilOpen(true)} 
                className="bg-primario text-white pl-4 pr-1.5 py-1.5 rounded-full flex items-center gap-3 font-semibold hover:bg-primario-hover transition-colors shadow-sm"
              >
                ...
                <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-texto-oscuro overflow-hidden border-2 border-transparent">
                  {usuarioActual.fotoPerfil ? (
                    <Image src={usuarioActual.fotoPerfil} alt="Perfil" width={32} height={32} className="w-full h-full object-cover" />
                  ) : (
      ...
      {/* NUEVO: Componente del Perfil que le pasamos el usuario logueado */}
      <PerfilModal 
        isOpen={isPerfilOpen} 
